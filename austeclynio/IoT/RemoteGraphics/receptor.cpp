...
Adafruit_ILI9341 Receptor::m_tela(TFT_CS, TFT_DC, TFT_MOSI, TFT_CLK, TFT_RST, TFT_MISO);
TouchScreen Receptor::m_touch(XMAX,YMAX,XMIN,YMIN,RESISTENCIA);
#endif
...
void Receptor::init(void){
  ...
  if((Receptor::m_estados & 0b00000010) == 0){
    ...
    if(
      ...
      Receptor::m_tela.begin(SSD1306_SWITCHCAPVCC,SCREEN_ADDRESS)
      #else
      true
      #endif
      ) {
      #ifdef TELA_TFT
        Receptor::m_tela.begin();
        Serial.println("ILI9341 Test!"); 
        // read diagnostics (optional but can help debug problems)
        uint8_t x = Receptor::m_tela.readcommand8(ILI9341_RDMODE);
        Serial.print("Display Power Mode: 0x"); Serial.println(x, HEX);
        x = Receptor::m_tela.readcommand8(ILI9341_RDMADCTL);
        Serial.print("MADCTL Mode: 0x"); Serial.println(x, HEX);
        x = Receptor::m_tela.readcommand8(ILI9341_RDPIXFMT);
        Serial.print("Pixel Format: 0x"); Serial.println(x, HEX);
        x = Receptor::m_tela.readcommand8(ILI9341_RDIMGFMT);
        Serial.print("Image Format: 0x"); Serial.println(x, HEX);
        x = Receptor::m_tela.readcommand8(ILI9341_RDSELFDIAG);
        Serial.print("Self Diagnostic: 0x"); Serial.println(x, HEX);
      #elif defined(TELA_LCD)
...
void Receptor::atualizarTela(void) {
  if((Receptor::m_estados & 0b00000010) && (Receptor::m_estados & 0b00000100)){
    ...
    #elif defined(TELA_TFT)
      Receptor::m_tela.setRotation(1);// modo paisagem
      for(uint8_t grafico = 0;grafico < GRAFICOS; grafico++){
        switch(grafico){
          case 0:
            Receptor::m_tela.setTextColor(COLOR1);
            break;
          case 1:
            Receptor::m_tela.setTextColor(COLOR2);
            break;
          case 2:
            Receptor::m_tela.setTextColor(COLOR3);
            break;
          case 3:
            Receptor::m_tela.setTextColor(COLOR4);
            break;
          case 4:
            Receptor::m_tela.setTextColor(COLOR5);
            break;
          default:
            Receptor::m_tela.setTextColor(ILI9341_WHITE);
        };
        Receptor::m_tela.print(F("A"));
        Receptor::m_tela.print(grafico,DEC);
        Receptor::m_tela.print(F(" = "));
        if(Receptor::m_estados & 0b00001000){
          Receptor::m_tela.println(digitalToVolts(grafico),2);
        }else{
          Receptor::m_tela.println(grafico,DEC);
        }
        Receptor::m_tela.print(F(" "));
      }
    #endif
    Receptor::m_tela.drawFastVLine(parametros[0],parametros[2],parametros[4],
    #ifdef TELA_LCD
    WHITE
    #elif defined(TELA_TFT)
    ILI9341_WHITE
    #endif
    );// desenha linha do eixo vertical
    Receptor::m_tela.drawFastHLine(parametros[0]-parametros[1],parametros[2],parametros[1],
    #ifdef TELA_LCD
    WHITE
    #elif defined(TELA_TFT)
    ILI9341_WHITE
    #endif
    );// desenha linha horizontal do valor máximo
    Receptor::m_tela.drawFastHLine(parametros[0]-parametros[1],(SCREEN_HEIGHT-1+parametros[2])/2,parametros[1],
    #ifdef TELA_LCD
    WHITE
    #elif defined(TELA_TFT)
    ILI9341_WHITE
    #endif
    );// desenha linha horizontal do valor médio
    Receptor::m_tela.drawFastHLine(parametros[0]-parametros[1],SCREEN_HEIGHT-1,parametros[1],
    #ifdef TELA_LCD
    WHITE
    #elif defined(TELA_TFT)
    ILI9341_WHITE
    #endif
    );// desenha linha horizontal do valor mínimo
    uint8_t gap = parametros[4]/10;
    uint8_t ticy = parametros[2];
    // desenha 4 linhas horizontais a partir do valor máximo
    for(uint8_t i = 0;i<5;i++){
      ticy += gap;
      Receptor::m_tela.drawFastHLine(parametros[0]-(parametros[1]/2),ticy,parametros[1]/2,
      #ifdef TELA_LCD
      WHITE
      #elif defined(TELA_TFT)
      ILI9341_WHITE
    ...
    // percorre a lista de valores do gráfico correspondente
      ...
      for(uint8_t indice = 0 ; indice < largura_grafico; indice++){
        ...
        if(indice > 0){
          Receptor::m_tela.drawLine(xa,ya,x,y,
          #ifdef TELA_LCD
          WHITE
          #elif defined(TELA_TFT)
          ILI9341_WHITE
    ...
    #elif defined(TELA_TFT)
      uint8_t xa[GRAFICOS],ya[GRAFICOS];// mantém os valores de x e y anteriores para cada gráfico
      for(uint8_t grafico = 0;grafico < GRAFICOS; grafico++){
        xa[grafico] = 0;
        ya[grafico] = 0;
      }
      for(uint8_t indice = 0 ; indice < largura_grafico; indice++){
        uint8_t x  = indice + parametros[0]+GAPGRAFICO;
        for(uint8_t grafico = 0;grafico < GRAFICOS; grafico++){
          uint8_t y  = Receptor::m_graficos[grafico][(indice+eixoxi) % parametros[3]];
          if(indice > 0){
            switch(grafico){
              case 0:
                Receptor::m_tela.drawLine(xa[grafico],ya[grafico],x,y,COLOR1);
                break;
              case 1:
                Receptor::m_tela.drawLine(xa[grafico],ya[grafico],x,y,COLOR2);
                break;
              case 2:
                Receptor::m_tela.drawLine(xa[grafico],ya[grafico],x,y,COLOR3);
                break;
              case 3:
                Receptor::m_tela.drawLine(xa[grafico],ya[grafico],x,y,COLOR4);
                break;
              case 4:
                Receptor::m_tela.drawLine(xa[grafico],ya[grafico],x,y,COLOR5);
                break;
              default:
                Receptor::m_tela.drawLine(xa[grafico],ya[grafico],x,y,ILI9341_WHITE);
            };
          }
          xa[grafico] = x;
          ya[grafico] = y;
