...
unsigned long counter;
  ...
  &tela to pass the display object, mainly used if multiple displays are connected to the MCU
...
void DrawBarChartV(Adafruit_SSD1306 &tela, double curval, double x , double y , double w, double h , double loval , double hival , double inc , double dig, String label, bool &Redraw)
{
  double stepval, my, level, i, data;

  if (Redraw) {
    Redraw = false;
    tela.fillRect(0, 0,  127 , 14, SSD1306_WHITE);
    tela.setTextColor(SSD1306_BLACK, SSD1306_WHITE);
    tela.setTextSize(1);
    tela.setCursor(2, 4);
    tela.println(label);
    ...
    for (i = 0; i <= h; i += stepval) {
      my =  y - h + i;
      tela.drawFastHLine(x + w + 1, my,  5, SSD1306_WHITE);
      // draw lables
      tela.setTextSize(1);
      tela.setTextColor(SSD1306_WHITE, SSD1306_BLACK);
      tela.setCursor(x + w + 12, my - 3 );
  ...
  tela.drawRect(x, y - h, w, h, SSD1306_WHITE);
  tela.fillRect(x, y - h, w, h - level,  SSD1306_BLACK);
  tela.drawRect(x, y - h, w, h, SSD1306_WHITE);
  tela.fillRect(x, y - level, w,  level, SSD1306_WHITE);
...
/*
  ...
  &telaisplay to pass the display object, mainly used if multiple displays are connected to the MCU
...
void DrawBarChartH(Adafruit_SSD1306 &tela, double curval, double x , double y , double w, double h , double loval , double hival , double inc , double dig, String label, bool &Redraw)
{
  double stepval, mx, level, i, data;

  if (Redraw) {
    Redraw = false;
    tela.fillRect(0, 0,  127 , 16, SSD1306_WHITE);
    tela.setTextColor(SSD1306_BLACK, SSD1306_WHITE);
    tela.setTextSize(1);
    tela.setCursor(2, 4);
    tela.println(label);
    ...
    for (i = 0; i <= w; i += stepval) {
      tela.drawFastVLine(i + x , y ,  5, SSD1306_WHITE);
      // draw lables
      tela.setTextSize(1);
      tela.setTextColor(SSD1306_WHITE, SSD1306_BLACK);
      tela.setCursor(i + x , y + 10);
  ...
  tela.fillRect(x + level, y - h, w - level, h,  SSD1306_BLACK);
  tela.drawRect(x, y - h, w,  h, SSD1306_WHITE);
  tela.fillRect(x, y - h, level,  h, SSD1306_WHITE);
  // up until now print sends data to a video buffer NOT the screen
  // this call sends the data to the screen
  tela.display();
...
void DrawDial(Adafruit_SSD1306 &tela, double curval, int cx, int cy, int r, double loval , double hival , double inc, double dig, double sa, String label, bool &Redraw) {
  ...
  if (Redraw) {
    ...
    for (i = 0; i <= sa; i += stepval) {
      ...
      tela.drawLine(ox, oy, ix, iy, SSD1306_WHITE);
      tela.setTextSize(1);
      tela.setTextColor(SSD1306_WHITE, SSD1306_BLACK);
      tela.setCursor(tx - 10, ty );
      data = hival - ( i * (inc / stepval)) ;
      tela.println(data, dig);
    }
    for (i = 0; i <= sa; i ++) {
      angle = ( i  * degtorad);
      angle = Offset - angle ;
      ox =  (r - 2) * cos(angle) + cx;
      oy =  (r - 2) * sin(angle) + cy;
      tela.drawPixel(ox, oy, SSD1306_WHITE);
    }
  }
  ...
  tela.fillTriangle (pix, piy, plx, ply, prx, pry, SSD1306_BLACK);

  // then draw the new needle
  tela.fillTriangle (ix, iy, lx, ly, rx, ry, SSD1306_WHITE);

  // draw a cute little dial center
  tela.fillCircle(cx, cy, 3, SSD1306_WHITE);
...
void DrawCGraph(Adafruit_SSD1306 &tela, double x, double y, double gx, double gy, double w, double h, double xlo, double xhi, double xinc, double ylo, double yhi, double yinc, double dig, String title, boolean &Redraw) {
  ...
  if (Redraw == true) {
    ...
    for ( i = ylo; i <= yhi; i += yinc) {
      ...
      if (i == 0) {
        tela.drawFastHLine(gx - 3, temp, w + 3, SSD1306_WHITE);
      }
      else {
        tela.drawFastHLine(gx - 3, temp, 3, SSD1306_WHITE);
      }
      tela.setCursor(gx - 27, temp - 3);
      tela.println(i, dig);
    }
    // draw x scale
    for (i = xlo; i <= xhi; i += xinc) {
      ...
      if (i == 0) {
        tela.drawFastVLine(temp, gy - h, h + 3, SSD1306_WHITE);
      }
      else {
        tela.drawFastVLine(temp, gy, 3, SSD1306_WHITE);
      }
      tela.setCursor(temp, gy + 6);
      tela.println(i, dig);
    }
  }
  ...
  tela.drawLine(ox, oy, x, y, SSD1306_WHITE);
  tela.drawLine(ox, oy - 1, x, y - 1, SSD1306_WHITE);
  ox = x;
  oy = y;
